...
if ( !empty($_GET["id"]) ){
    ...
    if(isset($_GET["xmode"])){
        ...
        $qres = mysqli_query($GLOBALS['dbh'],$query);
    ...
    $query = mysqli_query($GLOBALS['dbh'],'SELECT id_attr,attr_value
    ...
    if ($handle_action == "add"){
        ...
        foreach ($result_array AS $entry ){
            ...
            if(isset($entry["fk_show_class_items"])){
                $srvquery = mysqli_query($GLOBALS['dbh'],'SELECT config_class FROM ConfigClasses WHERE id_class='.$entry["fk_show_class_items"]);
