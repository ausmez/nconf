...
<?php
    $query = mysqli_query($GLOBALS['dbh'], "SELECT grouping FROM ConfigClasses WHERE nav_privs = 'user' AND grouping != '' GROUP BY grouping ORDER BY grouping");
...
<?php
    $query = mysqli_query($GLOBALS['dbh'], "SELECT grouping FROM ConfigClasses WHERE nav_privs = 'admin' AND grouping != '' GROUP BY grouping ORDER BY grouping");
    echo '<option value="">&nbsp;</option>';
    while($entry = mysqli_fetch_row($query)){
        echo '<option value="'.$entry[0].'"';
            if ( $entry[0] == $class_entry["grouping"] ) echo " selected";
