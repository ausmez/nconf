...
###
$query = 'SELECT id_attr, attr_name, friendly_name FROM ConfigAttrs WHERE fk_id_class="'.$fk_id_class.'" AND attr_name ="'.mysqli_real_escape_string($attr_name).'"';
...
if ($write2db == "yes"){
    ...
    if ($attr_id == "new"){
        ...
        if($result){
            # Get ID of insert:
            $new_id = mysqli_insert_id($GLOBALS['dbh']);
